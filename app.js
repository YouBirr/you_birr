...
app.listen(3002, async function(res,res){
  ...
  // await Creator.findOneAndDelete({username:"Nahom"});
  // await User.findOneAndDelete({username:"Adnan"});
  // await Package.findOneAndDelete({creator:"Nahom"})
  // await Post.findOneAndDelete({creator:"Nahom"})

  if(!youBirrBalance){
    const newYouBirrBalance = new YouBirrBalance({
      name:"youBirr"
    })
    await newYouBirrBalance.save();
  }
  console.log(youBirrBalance)
